...
export async function handleWebSearch(args) {
    ...
    try {
        ...
        const { stdout } = await execAsync(`curl -s -L "${ddgUrl}" --user-agent "Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36"`);
        ...
        const { stdout: htmlResults } = await execAsync(`curl -s -L "${liteUrl}" --user-agent "Mozilla/5.0 (X11; Linux x86_64)" | grep -oP '(?<=href=")[^"]+(?=")' | grep '^http' | head -${limit}`);
        const results = {
            query: searchQuery,
            search_type,
...
export async function handleNixSearch(args) {
    ...
    try {
        ...
        try {
            ...
            const { stdout } = await execAsync(`nix search ${nixChannel} ${searchTerm} --json 2>/dev/null || echo '{}'`, { timeout: 15000 });
            ...
            return {
                ...
                type,
                query: searchTerm,
                results: Object.entries(nixResults).map(([path, info]) => ({
                    path,
...
export async function handleGithubSearch(args) {
    ...
    try {
        ...
        const { stdout } = await execAsync(`curl -s -H "Accept: application/vnd.github.v3+json" "${apiUrl}"`);
        const data = JSON.parse(stdout);
        if (data.items) {
            return {
                success: true,
...
export async function handleTechNewsSearch(args) {
    ...
    try {
        ...
        if (source === "hackernews" || source === "all") {
            ...
            try {
                ...
                results.sources.push({
                    name: "Hacker News",
                    results: hn.hits?.slice(0, 5).map((hit) => ({
                        title: hit.title,
        ...
        if (source === "reddit" || source === "all") {
            ...
            try {
                const { stdout: redditData } = await execAsync(`curl -s -H "User-Agent: Mozilla/5.0" "${redditUrl}"`);
                const reddit = JSON.parse(redditData);
                results.sources.push({
                    name: "Reddit",
                    results: reddit.data?.children?.map((child) => ({
...
export async function handleDiscourseSearch(args) {
    ...
    try {
        ...
        return {
            ...
            results: data.posts?.slice(0, 10).map((post) => ({
                title: post.blurb,
