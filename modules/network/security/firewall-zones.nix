...
in
{
  ...
  config = mkIf cfg.enable {
        ...
        table inet filter {
          ...
          chain forward {
            ...
            ${optionalString (cfg.zones.isolated.enable && cfg.zones.isolated.denyInterzone) ''
              ...
              iifname { ${
                concatStringsSep ", " (map (x: ''"${x}"'') cfg.zones.isolated.interfaces)
              } } drop comment "Isolated zone - no interzone"
            ''}
    ...
    warnings =
      (optional (
        cfg.zones.dmz.enable && cfg.zones.dmz.interfaces == [ ]
      ) "DMZ zone is enabled but no interfaces are configured.")
      ++ (optional (
        cfg.zones.isolated.enable && cfg.zones.isolated.interfaces == [ ]
